...
library("RCurl") # used for getting the file directly from URL, getURL function is supported in this package
...
# to check how long an average text is, we use a command
mean(sapply(sapply(train_data_df$Text, strsplit, " "), length))
# train_data_df$Text, strsplit, " " - this will split the text by whitspace, now each word will be a variable in a list
#sapply(sapply(train_data_df$Text, strsplit, " "), length) - this will count the number of word
# finally, mean will calculate the average word in the text
...
library(SnowballC)
stopwords("english") # words which has no relevance in sentiment analysis like I, me, myself
corpus[1]$content

dtm <- DocumentTermMatrix(corpus)
dtm
# if we want to remove all the words that are not there in atleast 1% of the sentence, then we remove that word
# Command to remove that is supported in TM package, using removesparseterms
